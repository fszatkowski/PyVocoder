...
class Filter:
    ...
    def filter(self, signal: AudioSignal) -> SpectralSignal:
        ...
        filter_mask = frequency_bandwidth_mask(self.frequency, self.bandwidth, spectrum.data.shape[0], step)
...
class FilteredBandwidth:
    ...
    def noise_modulate(self, sample_rate: float, n_samples: float) -> SpectralSignal:
        ...
        mask = frequency_bandwidth_mask(self.frequency, self.bandwidth, int(n_samples), step)
...
class Vocoder:
    ...
    def modulate(self, audio: AudioSignal) -> AudioSignal:
        ...
        for f in self.filters:
            ...
            compressed_signal = FilteredBandwidth(f.frequency, f.bandwidth, max_amplitude)
        ...
        modulated_noises = [cs.noise_modulate(sample_rate, n_samples).data for cs in compressed_signals]
...
def frequency_bandwidth_mask(frequency:int, bandwidth:int, n_values: int, step: float) -> np.array:
